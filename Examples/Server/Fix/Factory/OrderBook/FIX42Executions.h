#ifndef Factory_OrderBook_FIX42Executions_dot_h
#define Factory_OrderBook_FIX42Executions_dot_h
...
namespace Fix::Factory::OrderBook {

//---------------------------------------------------------------------------------------------------------------------
// FIX 4.2 protocol message factory.
//---------------------------------------------------------------------------------------------------------------------
class FIX42Executions
{
public:
    template<typename Context>
    explicit FIX42Executions(Context& context)
      : m_session(context)
    ...
    FIX42::ExecutionReport execution(const Execution& execution, const Order& order)
    {
    ...
    FIX42::ExecutionReport cancellation(const Execution& execution, const Order& order)
    {
        auto omsg = make<FIX42::ExecutionReport>();
    ...
    FIX42::ExecutionReport correction(const Execution& execution, const Order& order)
    {
        auto omsg = make<FIX42::ExecutionReport>();
        ...
        omsg.set(FIX::ExecID(execution.executionID()));
        omsg.set(FIX::ExecType(FIX::ExecType_NEW));
        omsg.set(FIX::ExecTransType(FIX::ExecTransType_CORRECT));
        omsg.set(FIX::OrdStatus(std::underlying_type_t<Order::Status>(order.status())));
        omsg.set(FIX::OrderQty(order.quantity()));
        omsg.set(FIX::Price(order.price()));
        omsg.set(FIX::LastShares(execution.quantity()));
