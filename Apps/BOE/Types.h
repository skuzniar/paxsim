#ifndef BOE_Types_dot_h
#define BOE_Types_dot_h

#include "BOE/BOE2//Types.h"
...
public:
    ...
    Order(uint32_t clord_id, BOE2::BuySellIndicator side, uint32_t book_id, unsigned quantity, double price)
      : m_clord_id(clord_id)
      , m_side(side)
      , m_book_id(book_id)
      , m_quantity(quantity)
...
private:
    ...
    {
        static uint32_t m_id = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
        return m_id++;
    }

private:
    using Side = BOE2::BuySellIndicator;
