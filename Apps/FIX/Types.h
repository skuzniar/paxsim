#ifndef FIX_Types_dot_h
#define FIX_Types_dot_h
...
namespace tag      = hffix::tag;
namespace msg_type = hffix::msg_type;

//-----------------------------------------------------------------------------------------------------------------
// Incoming FIX message
//-----------------------------------------------------------------------------------------------------------------
class IMessage : public hffix::message_reader
{
public:
    IMessage(const char* buffer, size_t size)
      : message_reader(buffer, size)
    {
        // m_i = this->begin();
    }
    ...
    {
        return message_type()->value().as_string_view();
    }
    ...
    std::string_view operator[](int tag) const
    {
        auto dummy = this->begin();
        if (find_with_hint(tag, dummy)) {
            return dummy->value().as_string_view();
        }
        throw std::runtime_error("Tag " + std::to_string(tag) + " not present");
    }

    bool has(int tag) const
    {
        auto dummy = this->begin();
        return find_with_hint(tag, dummy);
    }
    ...
    friend std::ostream& operator<<(std::ostream& os, const IMessage& o)
    {
        return os << fixdump(o.data(), o.size());
    }

private:
    // mutable hffix::message_reader_const_iterator m_i;
};

//-----------------------------------------------------------------------------------------------------------------
// Outgoing FIX message
//-----------------------------------------------------------------------------------------------------------------
class OMessage : public hffix::message_writer
{
public:
    OMessage(char* buffer, size_t size, std::string_view beginString)
      : message_writer(buffer, size)
    {
        push_back_header(beginString);
    }

    void msgtype(std::string_view msgtype)
    {
        push_back_string(tag::MsgType, msgtype);
    }
    ...
    friend std::ostream& operator<<(std::ostream& os, const OMessage& o)
    {
        return os << fixdump(o.data(), o.size());
    }
};
