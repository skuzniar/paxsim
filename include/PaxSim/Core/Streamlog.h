...
public:
    hexdump_wrapper(const void* buf, std::size_t len, std::size_t chars_per_line = std::numeric_limits<std::size_t>::max())
      : m_buf(buf)
      , m_len(len)
    ...
    template<class CharT, class Traits>
    friend std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& os, const hexdump_wrapper& x)
    {
        x.print(os);
        return os;
    }
