...
public:
    ...
    IOSession(IOContext& iocontx, boost::asio::ip::tcp::socket& socket, Args&&... args)
      : m_iocontx(iocontx)
      , m_handler(m_ibuf, m_obuf, std::forward<Args>(args)...)
      ...
      , m_timer(iocontx.context())
    {
        ...
        if constexpr (has_init<IOHandler>) {
            m_handler.init();
        }
    }
...
private:
        ...
        try {
            ...
            if (m_ibuf.rsize() > 0 && m_obuf.below_hw_mark()) {
                do {
                    ...
                    if constexpr (has_eval<IOHandler>) {
                        m_handler.eval();
                    }
    ...
    {
        log << level::trace << ts << here << ' ' << e << std::endl;
